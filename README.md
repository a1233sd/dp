...
- сравнение с внешними источниками (локально сохраненными),
- правила исключений,
...
- Локальная БД `PostgreSQL` с персистентным хранением:
  - пользователей,
  - документов (`reference`, `submission`, `external`),
  - индексов документов (shingle hashes),
  - проверок и совпадений,
  - правил исключений.
- Поддержка двух типов контента: `text` и `code`.
...
- `Document`:
  ...
  - `external` — внешний источник (сохраненный локально текст + URL).
...
- `GET /documents` — список документов (фильтры: `kind`, `content_type`, `only_unique`)
- `GET /documents/{document_id}`

### Внешние источники
- `POST /external-sources` — добавить внешний источник (url + text)
...
2. Добавить эталоны (`POST /documents`, `kind=reference`) и при необходимости внешние источники (`POST /external-sources`).
...
4. Запустить проверку (`POST /checks`):
   ...
   - включить/выключить внешний контур и архив уникальных работ,
...
- Внешние источники сравниваются только если заранее добавлены через API (без онлайн-поиска).
